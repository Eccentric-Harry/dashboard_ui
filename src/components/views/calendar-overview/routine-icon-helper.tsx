...
const rules: { pattern: RegExp; icon: typeof ListChecks; color: string; bg: string }[] = [
  ...
  { pattern: /\b(pay|bill|finance|budget|bank|money|invest|tax|invoice|salary|payment|expense|subscription|renew|insurance|claim|reimbursement)\b/, icon: Wallet, color: '#16a34a', bg: '#f0fdf4' },
  ...
  { pattern: /\b(clean|organize|tidy|declutter|laundry|dishes|vacuum|mop|dust|wash|chore|housework|errand|repair|fix|maintenance|organizing)\b/, icon: ListChecks, color: '#475569', bg: '#f1f5f9' },
  { pattern: /\b(goal|objective|milestone|deadline|achieve|accomplish|target|quarterly|annual|kpi|okr|landmark)\b/, icon: Trophy, color: '#d97706', bg: '#fffbeb' },
...
export const getRoutineIconDetails = (item: {
  ...
  if (category === 'finance') return { icon: Wallet, color: '#16a34a', bg: '#f0fdf4' }
  ...
  if (type === 'REMINDER') return { icon: Bell, color: '#ea580c', bg: '#fff7ed' }
