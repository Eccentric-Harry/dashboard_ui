...
export interface GuestRequest {
  ...
  method: string;
  /** Serialized JSON request body, when there is one. */
  body?: string;
}
...
/**
 * Serves a request from the in-memory guest dataset. Returns `null` for requests that
 * must still reach the real network (the auth flow).
 */
export function resolveGuestRequest(request: GuestRequest): GuestResponse | null {
  ...
  if (urlStr.includes('/api/v1/mind/')) {
    ...
    const bodyOf = () => JSON.parse(typeof request.body === 'string' ? request.body : '{}');
  ...
  if (urlStr.includes('/api/v1/calendar/items') && (request.method || 'GET').toUpperCase() === 'POST') {
    const body = JSON.parse(typeof request.body === 'string' ? request.body : '{}');
    const newItem = {
      ...body,
